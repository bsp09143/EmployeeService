...
public class EmployeeService_Tests {
  ...
  String buildUrlEncodedFormEntity(String... params) {
    ...
    for (int i = 0; i < params.length; i += 2) {
      if (i > 0)
        result.append('&');
      result.append(URLEncoder.encode(params[i])).append('=')
  ...
  public void InsertEmployee_Using_Form_Should_Insert_New_Employees() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.empName").value("emp20")).andDo(print());
    // mockMvc.perform(
    // MockMvcRequestBuilders.post("/employee").contentType(MediaType.APPLICATION_FORM_URLENCODED)
    // .content(EntityUtils.toString(
    // new UrlEncodedFormEntity(Arrays.asList(new BasicNameValuePair("empId", "emp25"),
    ...
    // .content(EntityUtils.toString(
    // new UrlEncodedFormEntity(Arrays.asList(new BasicNameValuePair("empId", "emp21"),
  ...
  public void DeleteEmployee_Should_Delete_Employee() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.delete("/employee/emp20").accept(MediaType.APPLICATION_JSON))
  ...
  public void verifyInvalidEmployeeId() throws Exception {
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.get("/employee/emp35").accept(MediaType.APPLICATION_JSON))
  ...
  public void verifyInvalidEmployeeIdToDelete() throws Exception {
    mockMvc
        .perform(MockMvcRequestBuilders.delete("/employee/45").accept(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.errorCode").value(404))
